# Prompt Lovable — Idéal Contemporain

Colle le bloc de la partie 2 **tel quel** dans Lovable, comme tout premier message.

Lis d'abord la partie 1 : elle t'évitera de brûler tes crédits pour rien.

---

# 1. À lire avant de coller

Tes deux workspaces sont en **plan gratuit**. Les crédits se rechargent au compte-gouttes.

Ce prompt fait **reconstruire de zéro** en React un site qui existe déjà et qui fonctionne, en HTML/CSS/JS natif. Tu vas dépenser des crédits pour retrouver ce que tu as déjà.

### Ce que Lovable ne pourra pas reproduire fidèlement

La partie la plus délicate est le **moteur 3D procédural** : la géométrie du mobilier est construite à la volée en JavaScript, et les matières (veines du travertin, fil du bois, micro-grain des laques) sont générées par canvas. C'est ce qui permet de changer une dimension au centimètre en temps réel sans recharger de modèle.

Attends-toi à devoir itérer plusieurs fois sur cette partie. Prévois-y l'essentiel de ton budget.

### Quand Lovable vaut vraiment le coup

| Situation | Verdict |
|---|---|
| Mettre le site en ligne pour le montrer au client | ❌ GitHub Pages le fait gratuitement, aujourd'hui. Voir `GITHUB.md`. |
| Éditer visuellement plutôt qu'en code | ⚠️ Possible, mais tu paies la reconstruction complète. |
| Back-office, formulaire branché, base produits, comptes clients | ✅ **Là oui.** L'intégration Supabase apporte ce que Pages ne fait pas. |

**Ma recommandation :** publie d'abord sur GitHub Pages pour montrer le site cette semaine. Garde tes crédits pour le jour où le client demandera une vraie fonctionnalité back-end.

Si tu y vas quand même, la suite est prête.
...
# 3. Après le premier build

Vérifie dans cet ordre, et corrige avant d'aller plus loin :

1. La homepage en desktop **et** en mobile
2. Que la section 04 se charge en différé et ne bloque pas le premier rendu
3. `prefers-reduced-motion` — sur Mac : Réglages → Accessibilité → Affichage → Réduire les animations
4. Que le configurateur réagit bien **pendant** le glissement du curseur, pas seulement au relâchement

Pour les pages intérieures, les prompts de suivi sont dans `brief/prompts/01-iterations.md` — huit itérations, à envoyer une par une.
